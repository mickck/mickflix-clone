...
function Tv() {
  ...
  const { data, isLoading } = useQuery<IGetMoviesResult>(["tv, popular"], getTvs);
  console.log(data);
  const [back, setBack] = useState(false);
  const [index, setIndex] = useState(0);
  const [leaving, setLeaving] = useState(false);
  const navigator = useNavigate();
  const moreInfoMatch = useMatch("/mickflix-clone/tv/:tvId");
  ...
  const onBoxClicked = (tvId: number) => {
    navigator(`/mickflix-clone/tv/${tvId}`);
  };

  /*check exiting moreInfoMatch  */
  const clickedTv = moreInfoMatch?.params.tvId && data?.results.find((tv) => tv.id + "" === moreInfoMatch.params.tvId);
      ...
      {isLoading ? (
        ...
        <>
          <Banner bgphoto={makeImagePath(data?.results[0].backdrop_path || "")}>
            ...
            <InfoButton
              onClick={() => {
                navigator(`/mickflix-clone/tv/${data?.results[0].id}`);
              }}>
              More Info
            </InfoButton>
          </Banner>
          ...
          <AnimatePresence>
            {moreInfoMatch ? (
              <>
                ...
                <MoreInfoBox style={{ top: scrollY.get() + 50 }} layoutId={moreInfoMatch.params.tvId}>
                  {clickedTv && (
                    <>
